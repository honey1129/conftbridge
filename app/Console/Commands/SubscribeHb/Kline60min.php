...
class Kline60min extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'SubscribeHb:kline60min';
    /**
     * The console command description.
     *
     * @var string
     */
    protected $description = '处理60分钟数据（脚本）';
        ...
        Redis::subscribe(['vb:channel:ticker', 'Task:tickerNew'], function ($msg, $chan) use (&$coin_map, $coin_info)
        {
            ...
            if (!empty($item) && is_array($item)) {
                ...
                if (isset($coin_map[$item['code']])) {
                    ...
                    $new_time_index = date('H', $timestamp);
                    $datetime       = date('Y-m-d H:00:00', $timestamp);                            //年月日时分秒
                    ...
                    if ($coin_map[$item['code']]['old_time_index'] == null) {
                        ...
                        if ($coin_map[$item['code']]['old_time_index'] != $new_time_index) {
                            ...
                            if ($coin_info[$item['code']]['type'] == 1) {
                                try {
                                    //设置上一根时间
                                    $timestamp_db = $timestamp - 3600;
                                    $time_db      = date('H:00:00', $timestamp_db);
                                    $datetime_db  = date('Y-m-d H:00:00', $timestamp_db);
...
////                //重新调起客户端
                            run(function () use ($dataArr)
                            {
                                ...
                                $result = $redis->publish('vb:channel:newkline:minute60', json_encode($dataArr));
                            });
//
                        }
                    }
