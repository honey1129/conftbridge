...
class Kline15min extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'SubscribeHb:kline15min';
    /**
     * The console command description.
     *
     * @var string
     */
    protected $description = 'ws订阅15分钟数据（脚本）';
        ...
        Redis::subscribe(['vb:channel:ticker', 'Task:tickerNew'], function ($msg, $chan) use (&$coin_map, $coin_info)
        {
            ...
            if (!empty($item) && is_array($item)) {
                ...
                if (isset($coin_map[$item['code']])) {
                    ...
                    $new_time_index = bcsub($index, bcmod($index, 15));
...
////                //重新调起客户端
                            run(function () use ($dataArr)
                            {
                                ...
                                $result = $redis->publish('vb:channel:newkline:minute15', json_encode($dataArr));
                            });
//
                        }
                    }
