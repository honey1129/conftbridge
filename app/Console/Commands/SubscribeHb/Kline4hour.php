...
class Kline4hour extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'SubscribeHb:kline4hour';
    /**
     * The console command description.
     *
     * @var string
     */
    protected $description = '订阅四小时数据（脚本）';
        ...
        Redis::subscribe(['vb:channel:ticker', 'Task:tickerNew'], function ($msg, $chan) use (&$coin_map, $coin_info)
        {
            ...
            if (!empty($item) && is_array($item)) {
                ...
                if (isset($coin_map[$item['code']])) {
                    ...
                    $index          = date('H', $timestamp);
                    $new_time_index = bcsub($index, bcmod($index, 4));
                    $datetime       = date('Y-m-d ' . $new_time_index . ':00:00', $timestamp);
                    ...
                    if ($coin_map[$item['code']]['old_time_index'] == null) {
                        ...
                        if ($coin_map[$item['code']]['old_time_index'] != $new_time_index) {
                            ...
                            if ($coin_info[$item['code']]['type'] == 1) {
                                try {
                                    //设置上一根时间
                                    $timestamp_db      = $timestamp - 14400;
                                    $index_db          = date('H', $timestamp_db);
                                    $new_time_index_db = bcsub($index_db, bcmod($index_db, 4));
                                    $time_db           = date($new_time_index_db . ':00:00', $timestamp_db);
                                    $datetime_db       = date('Y-m-d ' . $new_time_index_db . ':00:00', $timestamp_db);
...
////                //重新调起客户端
                            run(function () use ($dataArr)
                            {
                                ...
                                $result = $redis->publish('vb:channel:newkline:hour4', json_encode($dataArr));
                            });
//
                        }
                    }
