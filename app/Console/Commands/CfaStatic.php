...
class CfaStatic extends Command
{
    use ClosePositions;

    //脚本命令
    protected $signature = 'cfa_static';
    //脚本名称
    protected $description = 'cfa 静态收益';
    ...
    public function handle()
    {
        ...
        try {
            ...
            UserPoolOrder::where(['status' => 1, 'order_type' => 1])->chunkById(100, function ($orders)
            {
                $totalDayStaticBalance = 0;
                foreach ($orders as $order) {
                    $user = User::where(['id' => $order->uid])->first();
                    if ($user->static_status) {
                        DB::beginTransaction();
                        $childPool = ChildPool::where(['id' => $order->child_pool_id])->first();
                        ...
                        $staticBalance = round($rate * 0.01 * $order->price, 6);

                        if ($order->pay_type == 2) {
                            // 租赁池主门票
                            // 池主
                            $chiZhuRate = $this->configs['zulin.chi_zhu_rate'] * 0.01;
                            $masterPool = MasterPool::where(['id' => $order->master_pool_id])->first();
                            $poolMasterUsdtAsset = UserAssets::getBalance($masterPool->uid, 3, 1, true);
                            $poolMasterBalance = round($staticBalance * $chiZhuRate, 6);
                            $this->writeBalanceLog($poolMasterUsdtAsset, 0, $poolMasterBalance, 14, '算力收益', 'computer power balance', $poolMasterUsdtAsset->pid, $poolMasterUsdtAsset->pname);

                            // 平台
                            $pingTaiAsset = UserAssets::getBalance(1, 3, 1, true);
                            $pingRate = $this->configs['zulin.pingtai_rate'] * 0.01;
                            $pingTaiBalance = $staticBalance * $pingRate;
                            $this->writeBalanceLog($pingTaiAsset, 0, $pingTaiBalance, 14, '算力收益', 'computer power balance', $pingTaiAsset->pid, $pingTaiAsset->pname);

                            // 用户
                            $userRate = 1 - $chiZhuRate - $pingRate;
                            $userAsset = UserAssets::getBalance($order->uid, 3, 1, true);
                            $userBalance = $staticBalance * $userRate;
                            $this->writeBalanceLog($userAsset, $order->id, $userBalance, 14, '算力收益', 'computer power balance', $userAsset->pid, $userAsset->pname);
                        } else {
                            $userAsset = UserAssets::where(['pid' => 3, 'uid' => $order->uid])->lockForUpdate()->first();
                            $this->writeBalanceLog($userAsset, $order->id, $staticBalance, 14, '算力收益', 'computer power balance', $userAsset->pid, $userAsset->pname);
                        }
                        DB::commit();
                        $totalDayStaticBalance += $staticBalance;
                        // 等级奖
                        $this->levelBalance($staticBalance, $user, $order);

                        // 直推奖
                        $this->recommendBalance($staticBalance, $user, $order);
                    }
                }

                // 今日静态收益累计
                SystemValue::where(['name' => 'day_static_balance'])->update([
                    'value' => $totalDayStaticBalance
                ]);

                // 分红今天收益累积
                SystemValue::where(['name' => 'fen_static_balance'])->increment('value', $totalDayStaticBalance);
            });
            Log::info('command cfa_static');
